...
def main(args):
    ...
    if args.dataset_name == 'imdb' or args.dataset_name == 'ag_news':
        output_dir = Path(os.path.join(args.ckpt_dir, 'finetune_{}_{}_lr{}_epochs{}'
    ...
    train_loader = ds.GeneratorDataset(source=train_dataset, shuffle=True)
    train_loader = train_loader.batch(batch_size=args.bsz)
    ...
    dev_loader = ds.GeneratorDataset(source=dev_dataset, shuffle=True)
    dev_loader = dev_loader.batch(batch_size=args.eval_size)

    # for test
    if args.do_test:
        test_dataset = utils.Huggingface_dataset(args, tokenizer, name_or_dataset=args.dataset_name,
                                                 subset=args.task_name, split='test')
        test_loader = ds.GeneratorDataset(source=test_dataset, shuffle=True)
        test_loader = test_loader.batch(batch_size=args.eval_size)
