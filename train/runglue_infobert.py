...
def feature_ranking(grad, cl=0.5, ch=0.9):
    ...
    norm = mindspore.norm(grad, dim=1)  # [seq_len]
    _, ind = mindspore.sort(norm)
    res = []
...
def get_seq_len(batch):
    lengths = mindspore.sum(batch['attention_mask'], dim=-1)
...
def train_mi_upper_estimator(mi_upper_estimator, outputs, batch=None):
    ...
    embeddings = mindspore.cat(embeddings)  # [-1, 768]   embeddings without masks
...
def get_local_robust_feature_regularizer(mi_estimator, args, outputs, local_robust_features):
    ...
    for i in range(0, len(local_embeddings), args.bsz):
        local_batch = mindspore.stack(local_embeddings[i: i + args.bsz])
        global_batch = mindspore.stack(global_embeddings[i: i + args.bsz])
        lower_bounds += mi_estimator(local_batch, global_batch),
    return -mindspore.stack(lower_bounds).mean()
        ...
        for model_inputs, labels in tqdm(train_loader):
            ...
            for astep in range(args.adv_steps):
                ...
                if args.adv_norm_type == "l2":
                    ...
                    denorm = mindspore.norm(delta_grad.view(delta_grad.size(0), -1), dim=1, p=float("inf")).view(-1, 1, 1)
                    denorm = mindspore.clamp(denorm, min=1e-8)
                    delta = (delta + args.adv_lr * delta_grad / denorm).detach()
                    if args.adv_max_norm > 0:
                        delta = mindspore.clamp(delta, -args.adv_max_norm, args.adv_max_norm).detach()
                embeds_init = word_embedding_layer(input_ids)
