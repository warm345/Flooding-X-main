...
def delta_embedding_init(args, model):
    ...
    delta_global_embedding = mindspore.zeros([vocab_size, hidden_size]).uniform_(-1, 1)
    # 30522 bert # 50265 roberta# 21128 bert-chinese
    dims = mindspore.tensor([hidden_size]).float()
    mag = args.adv_init_mag / mindspore.sqrt(dims)
...
def delta_lb_token(args, input_lengths, embeds_init, input_mask, delta_global_embedding, input_ids_flat, bs, seq_len):
    ...
    mag = args.adv_init_mag / mindspore.sqrt(dims)  # B
    delta_lb = mindspore.zeros_like(embeds_init).uniform_(-1, 1) * input_mask.unsqueeze(2)
    delta_lb = (delta_lb * mag.view(-1, 1, 1)).detach()
    gathered = mindspore.index_select(delta_global_embedding, 0, input_ids_flat)  # B*seq-len D
    delta_tok = gathered.view(bs, seq_len, -1).detach()  # B seq-len D
    denorm = mindspore.norm(delta_tok.view(-1, delta_tok.size(-1))).view(-1, 1, 1)  # norm in total degree?
    ...
    for epoch in range(args.epochs):
        ...
        for model_inputs, labels in tqdm(train_loader):
            ...
            for astep in range(args.adv_steps):
                ...
                denorm_lb = mindspore.norm(delta_lb_grad.view(bs, -1), dim=1).view(-1, 1, 1)
                denorm_lb = mindspore.clamp(denorm_lb, min=1e-8)
                denorm_lb = denorm_lb.view(bs, 1, 1)
                denorm_tok = mindspore.norm(delta_tok_grad, dim=-1)
                denorm_tok = mindspore.clamp(denorm_tok, min=1e-8)
                ...
                # calculate clip
                delta_norm_tok = mindspore.norm(delta_tok, p=2, dim=-1).detach()
                mean_norm_tok, _ = mindspore.max(delta_norm_tok, dim=-1, keepdim=True)
                ...
                if args.adv_max_norm > 0:
                    delta_norm = mindspore.norm(total_delta.view(bs, -1).float(), p=2, dim=1).detach()
